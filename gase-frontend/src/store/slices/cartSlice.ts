...
export const useCartTotalItems = () =>
  useSelector((state: RootState) => state.cart.totalItems);

export const useCartVisibility = () =>
  useSelector((state: RootState) => state.cart.isVisible);

export const useCartItemCount = (gasId: number) =>
  useSelector((state: RootState) => {
    const item = state.cart.items.find(item => item.gas.id === gasId);

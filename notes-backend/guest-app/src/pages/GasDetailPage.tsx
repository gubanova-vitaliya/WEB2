...
export const GasDetailPage: FC = () => {
  ...
  const handleImageError = (e: React.SyntheticEvent<HTMLImageElement, Event>) => {
    ...
    console.warn(`❌ Failed to load image in GasDetailPage: ${target.src}, falling back to default`);
  ...
  useEffect(() => {
    if (pageData?.image_url) {
      console.log(`🖼️ GasDetailPage [${pageData.title}]: image URL = ${pageData.image_url}`);
    } else if (pageData) {
      console.log(`⚠️ GasDetailPage [${pageData.title}]: no image URL, using default`);
    }
  }, [pageData?.image_url, pageData?.title]);
        ...
        <div className="gas-card">
          ...
          <div className="gas-image">
            <img
              ...
              onLoad={() => {
                console.log(`✅ GasDetailPage [${pageData.title}]: image loaded successfully from ${pageData.image_url || getDefaultImage()}`);
              }}
              loading="lazy"
            />
          </div>

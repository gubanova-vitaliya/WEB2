...
export const GasCard: FC<GasCardProps> = ({ gas, onCardClick }) => {
  ...
  React.useEffect(() => {
    if (gas.image_url) {
      console.log(`🖼️ GasCard [${gas.title}]: image URL = ${gas.image_url}`);
    } else {
      console.log(`⚠️ GasCard [${gas.title}]: no image URL, using default`);
    }
  }, [gas.image_url, gas.title]);

  const imageSrc = gas.image_url || getDefaultImage();
  ...
  return (
    <Card className="gas-card">
      <Card.Img
        ...
        onLoad={() => {
          console.log(`✅ GasCard [${gas.title}]: image loaded successfully from ${imageSrc}`);
        }}
      />
      <Card.Body>
